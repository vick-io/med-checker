...
$MYSQL_BIN -vvv -u $user -p$password -h$dbserver $db_name < Table_scripts_mysql_rxn.sql >> mysql.log 2>&1
if [ $? -ne 0 ]; then ef=1; fi

if [ $ef -ne 1 ]
then
echo "    Load Data ... `/bin/date`" >> mysql.log 2>&1
$MYSQL_BIN -vvv -u $user -p$password -h$dbserver $db_name < Load_scripts_mysql_rxn_unix.sql >> mysql.log 2>&1
if [ $? -ne 0 ]; then ef=1; fi
fi
